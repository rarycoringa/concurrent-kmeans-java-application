...
bench-micro dataset_path version="v5" filter="KMeansBenchmark":
...
bench-macro-run version="v5":
    #!/usr/bin/env bash
    set -e
    mkdir -p results/macro/{{version}}
    for jmx in jmeter/ParallelGC-threads-*.jmx; do
        name=$(basename "$jmx" .jmx)
        result="$(pwd)/results/macro/{{version}}/${name}-results.jtl"
        echo "=== $name ==="
        GC_ALGO="-XX:+UseParallelGC" JVM_ARGS="-Xmx4g" \
...
profile dataset_path version="v5":
...
bench-stress version="v5" filter="JCS":
